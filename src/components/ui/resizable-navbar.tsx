    ...
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-white font-bold dark:text-white-300 [text-shadow:2px_1px_2px_#000000]  "
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-teal-700 dark:bg-teal-700"
            />
          )}
